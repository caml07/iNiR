...
QtObject {
    ...
    readonly property var studio: [
        ...
        { target: "pieces", group: "Size", label: "Shape", description: "The profile of every bubble, bar and satellite. Circles are the Island's own; squircles and squares read like app tiles.", path: "iris.appearance.theme.pieceShape", kind: "choice", fallback: "circle", choices: [{label:"Circle",value:"circle",glyph:"circle"},{label:"Squircle",value:"squircle",glyph:"crop_square"},{label:"Square",value:"square",glyph:"square"}] },
        ...
        { target: "pieces", group: "Opening bodies", label: "Join to opener", description: "When settled, let a card or Control Center become one silhouette with the bubble or Island that opened it.", path: "iris.appearance.surfaces.cards.joinOrigin", kind: "switch", fallback: true },
    ...
    readonly property var behaviour: [
        ...
        { section: "dock", group: "Visibility", label: "Reserve space for windows", description: "Keep tiled windows clear of the visible Dock. Turn this off to let the Dock sit over them.", path: "iris.dock.reserveSpace", visibleWhen: "!iris.dock.autoHide", kind: "switch", fallback:true },
        ...
        { section: "sidebars", group: side === "left" ? "Focus · left" : "Today · right", label: "Reserve space for windows", description: "Keep tiled windows clear while this panel is kept open. Turn this off to let the panel overlay them.", path: "iris.sidebars." + side + ".reserveSpace", visibleWhen: "iris.sidebars." + side + ".pinned", kind: "switch", fallback:true }
    ])))

    readonly property var sectionOrder: ["bar", "player", "bubbles", "dock", "appearance", "desktop", "sidebars", "surfaces", "system"]
    function sectionOf(spec: var): string {
        const path = String(spec.path ?? "")
        switch (spec.target) {
        case "island": return "bar"
        case "pieces": return "bubbles"
        case "bodies": return path.startsWith("iris.player.") ? "player" : "surfaces"
        case "places": return path.startsWith("iris.wallpaper.") || path.includes(".gallery.") ? "desktop" : "surfaces"
        case "transients": return "surfaces"
    ...
    readonly property var settings: {
        const known = new Set(root.behaviour.map(spec => spec.path))
        const look = []
        for (const spec of root.studio) {
            if (known.has(spec.path)) continue
            known.add(spec.path)
            look.push(Object.assign({ section: root.sectionOf(spec) }, spec))
        }
        const out = []
        for (const section of root.sectionOrder) {
            for (const spec of root.behaviour) if (spec.section === section) out.push(spec)
