...
| Interactive Web Wallpaper | base | XBPS `qt6-webengine`, `layer-shell-qt`; Quickshell as fallback QML runner | per-screen child process | Engineering closure: providers, runner fallback, Doctor repair, and local guards pass; clean-VM runtime validation pending | final fat-check |
...
| Legacy Super-tap opt-in | optional/legacy | XBPS `python3-evdev` + installed helper | systemd, Turnstile, or runsvdir by predicate | Engineering closure: runit/Turnstile service generation and cleanup are tested locally; clean-VM opt-in runtime validation pending | final fat-check |
