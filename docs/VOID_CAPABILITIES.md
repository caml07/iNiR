...
| OCR | toolkit | XBPS Tesseract language packages + pinned `tessdata_fast` vertical models | direct process + `tesseract` adapter | VM validated: required horizontal packages, verified vertical models, model-load smoke tests, repair, and idempotency | PR5.1 |
...
| Shell desktop parity | base/fonts/theme | XBPS `plasma-integration`, `kf6-syntax-highlighting`, iNiR Turnstile env handoff, Void distro asset | Niri + Quickshell user session | VM validated: valid KDL, KDE platform integration, sidebars, Void icon, refreshed `NIRI_SOCKET`, and end-to-end `Mod+Q` input path | PR5.5 |
