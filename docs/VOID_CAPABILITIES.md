...
| Network | base | XBPS `NetworkManager` | runit service with interactive migration from Void's base dhcpcd/wpa_supplicant stack | VM validated: activation, group, runit process, D-Bus ownership, and `nmcli` reports `connected` after reboot. External-disk validation exposed the missing clean-install migration; the provider now performs an explicit, rollback-safe handoff and Doctor detects a stopped NetworkManager service | PR4.0 + external-disk closure |
